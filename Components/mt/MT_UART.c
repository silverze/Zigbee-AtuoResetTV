...
void MT_UartInit ()
{
  ...
  /*¥Æø⁄ªÿµ˜∫Ø ˝ ¥˙¬Î∏˙◊Ÿ£∫ osal_start_system -> osal_run_system()->
  Hal_ProcessPoll()-> HalUARTPoll()-> ....
  */
  
#if defined (ZTOOL_P1) || defined (ZTOOL_P2)   //‘§¥¶¿Ì∆˜÷–∂®“Â¡ÀZTOOL_P1
  //uartConfig.callBackFunc         = MT_UartProcessZToolData; 
  uartConfig.callBackFunc         = AutoResetTVApp_UartProcessData; 
 ...
 * @fn      putchar
 *
 * @brief   Ω´printf∫Ø ˝÷ÿ∂®œÚ÷¡µ•∆¨ª˙¥Æø⁄
 *
 */
__near_func int putchar(int c)
{
  uint8 str = c;   
  HalUARTWrite(MT_UART_DEFAULT_PORT, &str, 1);
  return (c);
}
